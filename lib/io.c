...
{
// 	ecture de la valeur sur le port gpio après masquage. Les bits correspondants à un 0 du
// masque sont lus comme des 0.
...
// niveau logique haut (resp. bas) sur la broche correspondante du port. Les bits à 0 du masque
// mask permettent de ne pas modifier la valeur des broches correspondantes. Aucun décalage
// n'est appliqué à val par la fonction.
	gpio->ODR &= ~(mask);
	gpio->ODR |=  val & mask;  
}
...
{
// 	écriture de val sur le port gpio après masquage. Un bit à 1 (resp. 0) dans val écrit un
// niveau logique bas (resp. haut) sur la broche correspondante du port. Les bits à 0 du masque
// mask permettent de ne pas modifier la valeur des broches correspondantes. Aucun décalage
// n'est appliqué à val par la fonction.
	gpio->ODR &= ~(mask);
	gpio->ODR |= ~val & mask;  
